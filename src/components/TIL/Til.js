...
const Til = ({ viewerId, viewerIds, slide }) => {
  ...
  const { data: tilSubscriptionData, error: vhutSubscription_error } =
    ...
    useEffect(()=>{
      if(tilSubscriptionData?.tilStatus?.message === "Til is completed"){
        console.log(tilSubscriptionData?.tilStatus?.data.tumor_cords);
          setTilCords(tilSubscriptionData?.tilStatus?.data?.tils_cords);
        setTumorCords(tilSubscriptionData?.tilsStatus?.data?.tumor_cords);
        setStromaCords(tilSubscriptionData?.tilsStatus?.data?.stroma_cords);
  ...
  const roi3 = stromaCords?.map((stroma_cord) => {
      const points3 = stroma_cord.map((point3) => ({
        ...
        if(tilSubscriptionData?.tilStatus?.data?.tils_cords?.length>0 || tilSubscriptionData?.tilStatus?.data?.stroma_cords?.length>0 || tilSubscriptionData?.tilStatus?.data?.tumor_cords?.length>0){
            ...
            const canvas = fabricOverlay.fabricCanvas();
            const color = "#2Aff00";
            const roi = tilSubscriptionData?.tilStatus?.data?.tils_cords.flat(2).map((TIL_cord) => {
                return new fabric.Rect({
                    top: TIL_cord[1],
                    // bottom:TIL_cord[1],
                    left: TIL_cord[0],
